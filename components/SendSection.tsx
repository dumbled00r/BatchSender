...
const SendSection = () => {
    ...
    return (
        <>
            ...
            >
                <option value="etherSameAmount">
                    Transfer Ether same amount
                </option>
                <option value="etherDifferentAmount">
                    Transfer Ether different amount
                </option>
                <option value="tokenSameAmount">
                    Transfer Token same amount
                </option>
                <option value="tokenDifferentAmount">
                    Transfer Token different amount
                </option>
            </select>
            ...
            {transferMode === 'etherDifferentAmount' && (
                <>
                    ...
                    <br></br>
                    <button
                        onClick={sendEtherDifferentAmount}
                        className={styles['button-3']}
                    >
                        Send
                    </button>
            ...
            {isConfirming && (
                <div style={{ marginTop: '20px', color: 'rgb(173, 216, 230)' }}>
                    Waiting for confirmation...
                </div>
            )}
            {isConfirmed && (
                <div style={{ marginTop: '20px', color: 'rgb(54, 180, 34)' }}>
                    Transaction confirmed.
                </div>
            )}
            {error && (
                <div style={{ color: 'rgb(255, 182, 193)' }}>
                    Error: {(error as BaseError).shortMessage || error.message}
                </div>
            )}
            {(transferMode === 'tokenSameAmount' ||
                transferMode === 'tokenDifferentAmount') && (
                <h1>Coming soon...</h1>
            )}
            <ToastContainer
                position="top-center"
